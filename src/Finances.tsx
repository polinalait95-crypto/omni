    ...
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginBottom: 14 }}>
        <div className="card">
          ...
          <div style={{ fontFamily: "var(--mono)", fontSize: 26, fontWeight: 600, color: "var(--green)", marginTop: 8 }}>+{fmt(sum.income)} {cur()}</div>
        </div>
        <div className="card">
          <div style={{ fontSize: 12, color: "var(--dim)" }}>Расход</div>
          <div style={{ fontFamily: "var(--mono)", fontSize: 26, fontWeight: 600, color: "var(--red)", marginTop: 8 }}>−{fmt(sum.expense)} {cur()}</div>
        </div>
        <div className="card">
          <div style={{ fontSize: 12, color: "var(--dim)" }}>Баланс</div>
          <div style={{ fontFamily: "var(--mono)", fontSize: 26, fontWeight: 600, marginTop: 8 }}>{balance >= 0 ? "+" : "−"}{fmt(Math.abs(balance))} {cur()}</div>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 14, alignItems: "start" }}>
        ...
        <div className="card">
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
            ...
            <input className="fin-in" placeholder={"Сумма " + cur()} value={amount} onChange={(e) => setAmount(e.target.value)} style={inp(70)} />
          ...
          {txns.map((t) => (
            <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 0", borderBottom: "1px solid var(--line)" }}>
              ...
              <div style={{ fontFamily: "var(--mono)", fontWeight: 600, fontSize: 14, color: t.direction === "income" ? "var(--green)" : "var(--text)" }}>
                {t.direction === "income" ? "+" : "−"}{fmt(t.amount)} {cur()}
              </div>
              ...
              return (
                <div key={e.id} style={{ marginBottom: 14 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5, marginBottom: 7 }}>
                    ...
                    <span style={{ fontFamily: "var(--mono)", color: "var(--dim)" }}>{fmt(e.used)} / {fmt(e.limit)} {cur()}</span>
                  </div>
              ...
              {sum.by_category.map((c, i) => (
                <div key={c.category} style={{ marginBottom: 11 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5, marginBottom: 6 }}>
                    ...
                    <span style={{ fontFamily: "var(--mono)", color: "var(--dim)" }}>{fmt(c.total)} {cur()}</span>
                  </div>
...
function TxnEdit({ txn, onClose, onSave }: {
  ...
  return (
    <Modal title="Редактировать операцию" onClose={onClose}>
      ...
      <div className="field"><label>Сумма, {cur()}</label><input value={amount} onChange={(e) => setAmount(e.target.value)} autoFocus /></div>
