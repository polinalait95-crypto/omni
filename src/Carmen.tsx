...
import { respond, type CardMeta } from "./carmenBrain";

const rub = (m: number) => (m / 100).toLocaleString("ru", { maximumFractionDigits: 0 }) + " " + cur();
...
export default function Carmen({ onGoto, toast }: { onGoto: (v: "tasks" | "fin" | "subs" | "cal") => void; toast: (t: string) => void }) {
  ...
  async function send() {
    ...
    try {
      ...
      // не «зависаем на думает…» — показываем, что пошло не так
      const msg = "Не удалось выполнить: " + (e instanceof Error ? e.message : String(e));
      if (sid) { try { await api.addMessage(sid, "assistant", msg); await loadMsgs(sid); } catch { /* ignore */ } }
      toast("Ошибка при обработке запроса");
    } finally {
      setBusy(false); // сбрасывается ВСЕГДА, даже при ошибке
    }
  }
