...
async function mock<T>(cmd: string, a?: Record<string, unknown>): Promise<T> {
  ...
  switch (cmd) {
    ...
    case "add_message": {
      ...
      messages.push({ ...m, ...({ session_id: a!.sessionId } as any) });
    ...
    case "list_messages":
      return messages.filter((m: any) => m.session_id === a!.sessionId).sort((x, y) => x.created_at - y.created_at) as unknown as T;
    case "get_prefs":
    ...
    case "create_rule": {
      const r: DocRule = { id: crypto.randomUUID(), parent_id: (a!.parentId as string) ?? null, name: String(a!.name), file_types: (a!.fileTypes as string) ?? "[]", action: (a!.action as string) ?? null, prompt: (a!.prompt as string) ?? null, is_folder: Boolean(a!.isFolder), sort_order: rules.length };
      rules.push(r);
      return r as unknown as T;
    }
    case "update_rule": {
      const r = rules.find((x) => x.id === a!.id);
      if (r) { r.name = String(a!.name); r.file_types = String(a!.fileTypes); r.action = (a!.action as string) ?? null; r.prompt = (a!.prompt as string) ?? null; }
      return undefined as unknown as T;
    }
    case "delete_rule": {
    ...
    case "update_transaction": {
      ...
      if (t) { t.direction = a!.direction as Direction; t.amount = a!.amount as number; t.title = String(a!.title); t.category = (a!.category as string) ?? null; t.occurred_at = a!.occurredAt as number; }
      return undefined as unknown as T;
    }
    case "update_subscription": {
      const s = subs.find((x) => x.id === a!.id);
      if (s) { s.title = String(a!.title); s.amount = (a!.amount as number) ?? null; s.period = String(a!.period); s.next_renewal = (a!.nextRenewal as number) ?? null; s.notify_days = String(a!.notifyDays); s.url = (a!.url as string) ?? null; }
      return undefined as unknown as T;
    }
    case "search": {
    ...
    case "add_transaction": {
      const t: Transaction = {
        ...
        method: (a!.method as string) ?? null, occurred_at: (a!.occurredAt as number) ?? Date.now(),
      };
      txns.unshift(t);
    ...
    case "add_subscription":
      subs.push({
        ...
        currency: "RUB", period: (a!.period as string) ?? "month", next_renewal: (a!.nextRenewal as number) ?? null,
        status: "active", auto_renew: true, url: (a!.url as string) ?? null, notify_days: (a!.notifyDays as string) ?? "[3]",
      });
      return undefined as unknown as T;
    ...
    case "list_attachments":
      return atts.filter((x) => x.entity_id === a!.entityId).map(({ bytes, ...m }) => (void bytes, m)) as unknown as T;
    case "save_attachment": {
      const at: MockAtt = {
        id: crypto.randomUUID(), entity_id: String(a!.entityId), file_name: String(a!.fileName),
...
export const api = {
  ...
  addMessage: (sessionId: string, role: string, content: string, meta = "{}") =>
    call<ChatMessage>("add_message", { sessionId, role, content, meta }),
  listMessages: (sessionId: string) => call<ChatMessage[]>("list_messages", { sessionId }),
  ...
  createRule: (r: { parentId: string | null; name: string; fileTypes: string; action: string | null; prompt: string | null; isFolder: boolean }) =>
    call<DocRule>("create_rule", { parentId: r.parentId, name: r.name, fileTypes: r.fileTypes, action: r.action, prompt: r.prompt, isFolder: r.isFolder }),
  updateRule: (id: string, name: string, fileTypes: string, action: string | null, prompt: string | null) =>
    call<void>("update_rule", { id, name, fileTypes, action, prompt }),
  ...
  addTransaction: (t: {
    ...
    category: t.category ?? null, method: t.method ?? null, occurredAt: t.occurredAt ?? null,
  }),
  ...
  updateTransaction: (id: string, t: { direction: Direction; amount: number; title: string; category: string | null; occurredAt: number }) =>
    call<void>("update_transaction", { id, direction: t.direction, amount: t.amount, title: t.title, category: t.category, occurredAt: t.occurredAt }),
  ...
  createPage: (title: string, body: string | null = null, parentId: string | null = null) =>
    call<Page>("create_page", { title, body, parentId }),
  ...
  addSubscription: (s: {
    ...
    nextRenewal: s.nextRenewal ?? null, notifyDays: s.notifyDays ?? "[3]", url: s.url ?? null,
  }),
  ...
  updateSubscription: (id: string, s: { title: string; amount: number | null; period: string; nextRenewal: number | null; notifyDays: string; url: string | null }) =>
    call<void>("update_subscription", { id, title: s.title, amount: s.amount, period: s.period, nextRenewal: s.nextRenewal, notifyDays: s.notifyDays, url: s.url }),

  // вложения (файлы на диске)
  listAttachments: (entityId: string) => call<Attachment[]>("list_attachments", { entityId }),
  saveAttachment: (entityId: string, fileName: string, mime: string | null, bytes: number[]) =>
    call<Attachment>("save_attachment", { entityId, fileName, mime, bytes }),
