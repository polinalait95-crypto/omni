...
export default function CmdK({ reload, onFinance, toast, onClose }: {
  ...
  async function run() {
    ...
    if (intent.kind === "task") {
      ...
      onFinance();
      toast(`${intent.direction === "income" ? "Доход" : "Расход"} записан: ${intent.amount} ${cur()}`);
      onClose();
    } else {
        ...
        {intent?.kind === "finance" && (
          <div className="cmdk-intent">
            ...
            <div className="parsed">
              {intent.direction === "income" ? "+" : "−"}<b>{intent.amount} {cur()}</b> · {intent.title}
            </div>
          ...
          {intent?.kind === "finance" && (
            <>
              ...
              <div className="cmdk-item primary sel" onClick={run}>
                ...
                <div className="ci-txt"><b>{intent.direction === "income" ? "Записать доход" : "Записать расход"}</b><small>{intent.amount} {cur()} · {intent.title}</small></div>
              </div>
            </>
          )}
          {intent?.kind === "search" && (
