    ...
    <>
      <div className="toolbar">
        ...
        <div style={{ fontSize: 12.5, color: "var(--dim)", marginRight: 6 }}>
          В месяц: <b style={{ fontFamily: "var(--mono)", color: "var(--text)" }}>{fmt(monthlyTotal)} {cur()}</b>
        </div>
      ...
      {adding && (
        <div className="card" style={{ marginBottom: 14, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <input placeholder={"Стоимость " + cur()} value={amount} onChange={(e) => setAmount(e.target.value)} style={sub_in(100)} />
...
function SubEdit({ sub, onClose, onSave }: {
  ...
  return (
    <Modal title="Редактировать подписку" onClose={onClose}>
      ...
      <div className="field"><label>Стоимость, {cur()}</label><input value={amount} onChange={(e) => setAmount(e.target.value)} /></div>
...
function Card({ sub, onStatus, onEdit, onRemove, toast }: { sub: Subscription; onStatus: (id: string, s: SubStatus) => void; onEdit: () => void; onRemove: () => void; toast: (t: string) => void }) {
  ...
  return (
    <div className="card" style={{ padding: 15 }}>
      ...
      {sub.amount != null && <div style={{ fontFamily: "var(--mono)", fontSize: 22, fontWeight: 600 }}>{fmt(sub.amount)} {cur()}</div>}
      {sub.next_renewal && (
