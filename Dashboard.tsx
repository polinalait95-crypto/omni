  ...
  return (
    <>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 14 }}>
        ...
        {kpi("Баланс месяца", `${balance >= 0 ? "+" : "−"}${fmt(Math.abs(balance))} ${cur()}`, "доход минус расход", balance >= 0 ? "var(--green)" : "var(--red)")}
        {kpi("Подписки / мес", `${fmt(monthly)} ${cur()}`, `${subs.filter((s) => s.status !== "cancelled").length} активных`)}
            ...
            {sum.by_category.slice(0, 4).map((c) => {
              ...
              return (
                <div key={c.category} style={{ marginBottom: 9 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5, marginBottom: 4 }}><span>{c.category}</span><span style={{ fontFamily: "var(--mono)", color: "var(--dim)" }}>{fmt(c.total)} {cur()}</span></div>
