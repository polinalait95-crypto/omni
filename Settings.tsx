...
export default function Settings({ prefs, onChange }: { prefs: Prefs; onChange: (p: Prefs) => void }) {
  ...
  return (
    <div className="setting-block" style={{ maxWidth: 620 }}>
      ...
      <div className="sec-title">Валюта</div>
      <div className="set-row">
        <div className="sl"><b>Валюта по умолчанию</b><small>Символ подставляется во всех разделах</small></div>
        <select className="field-select" value={ui.currency} onChange={(e) => setUi({ currency: e.target.value })}>
