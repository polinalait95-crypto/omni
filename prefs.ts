...
export interface UiConf { wallpaper: string; font: string; fontSize: number; currency: string }
...
export function parseUi(p: Prefs): UiConf {
  try {
    ...
    return { wallpaper: u.wallpaper ?? "none", font: u.font ?? "Inter", fontSize: u.fontSize ?? 14, currency: u.currency ?? "BYN" };
  } catch {
    return { wallpaper: "none", font: "Inter", fontSize: 14, currency: "BYN" };
  }
}
